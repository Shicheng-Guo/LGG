champ.QC<-function (beta = myLoad$beta, pheno = myLoad$pd$Sample_Group, 
          mdsPlot = TRUE, densityPlot = TRUE, dendrogram = TRUE, PDFplot = TRUE, 
          Rplot = TRUE, Feature.sel = "None", resultsDir = "./CHAMP_QCimages/") 
{
  message("[===========================]")
  ...
  if (densityPlot) {
    if (Rplot) 
      densityPlot(beta, sampGroups = pheno, main = paste("Density plot of raw data (", 
                  ...
                  xlab = "Beta")
    if (PDFplot) {
      pdf(paste(resultsDir, "raw_densityPlot.pdf", 
  ...
  if (dendrogram) {
    ...
    if (PDFplot) {
      pdf(paste(resultsDir, "raw_SampleCluster.pdf", 
                sep = "/"), width = floor(log(ncol(beta)) * 
                                            3), height = floor(log(ncol(beta)) * 2))
      plot(dend, center = TRUE, main = paste("All samples before normalization (", 
                                             nrow(beta), " probes)", sep = ""))
      legend("topright", fill = MyColor, legend = names(MyColor))
      dev.off()
